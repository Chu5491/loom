...
## 재계획 (2026-06-10) — 위 P5/P6을 대체

**상태:** P1~P4 완료(office-as-code · 런엔진+sqlite · Talk · 하네스 자동발화). **휴대성(구 P5)은 보류.** 디테일·UX 개편이 더 급함.

**확정 결정:**
- **프로젝트 = 작업 디렉토리 + 공유 오피스.** office(에이전트·규약·스킬·MCP·하네스)는 전역 공유 = "내 팀". 프로젝트 = 등록된 로컬 디렉토리 목록 = "팀이 일할 곳". run/스레드가 프로젝트를 cwd로 타겟. (per-project office 아님 — 단순·휴대성 일치.)
- **하네스 = 워크플로우 유지 + 스마트 디스패치 추가.** 명시적 edges 그래프는 그대로 두고, "작업 설명 → 적합 에이전트 자동 선택"(스킬/설명 매칭 또는 LLM 라우터)을 @auto 같은 별도 기능으로.

**새 단계 (프로젝트가 토대 → 나머지 4묶음):**

| 단계 | 내용 | 검증 |
|---|---|---|
| P5 프로젝트 | 로컬 디렉토리 레지스트리(등록·목록·선택, data/projects.json 또는 sqlite). 헤더/사이드에 프로젝트 셀렉터. run/스레드가 프로젝트 cwd에서 실행. 스레드를 프로젝트로 스코프. | 두 프로젝트 등록 → 각각서 run → 올바른 cwd, 스레드 분리 |
| P6 Office UX | 리스트 압축(rule/skill 한 줄 + 펼침/접기). 에이전트 **이름·모델 필수** + 아바타=**CLI 아이콘**(AdapterIcon 재사용). 모델 검증. MCP raw JSON→폼. (선택) 스킬 as 폴더(references). | 30개 규약도 한눈에, 에이전트 카드에 brand 아이콘 |
| P7 대화 강화 | 컴포저 멘션 확장: @agent + **@skill** + **@file**(프로젝트 파일 검색). 참여/가능 에이전트 패널(누가 도는지·누구 부를 수 있는지). 핸드오프 가시화 강화. | @file로 프로젝트 파일 첨부, 에이전트 패널 표시 |
| P8 연결 재배치 + 디테일 | 헤더에 인증된 CLI 인디케이터. 연결탭=테스트 도구로 슬림화. **cancel 버튼**(엔진 있음), **manual/ask 수동발화 버튼**(엔진 보강 필요), 비용 롤업. | 헤더 인증 표시, 실행 중 취소, ask 엣지 수동 발화 |
| P9 스마트 디스패치 | "작업→적합 에이전트" 자동 선택(스킬/설명 매칭 → LLM 라우터). 하네스와 공존. | 작업 설명만으로 적합 에이전트 발화 |

**미해결 디테일(틈틈이):** 스킬 references(폴더화), Talk 다중 스레드, ask/manual UI, 비용 합계, MCP 폼, 모델 검증. → 위 단계에 흡수.

**규율 변경:** "화면 3개" 완화 가능 — 프로젝트 셀렉터/패널이 늘면 레일/사이드 재도입 허용. 단 office-as-code·자동주입 금지·CLI root 불가침·정의는git/기록은로컬 5원칙은 불변.
