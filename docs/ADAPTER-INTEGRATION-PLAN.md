...
| ✅(5b81a0f) | `-o stream-json` + droid 스트림 이벤트 파싱 | `index.ts`(buildCommand)·`parse.ts`(droid 분기) | 🟡 | **완료** — custom 로컬모델로 전체 스키마 실측 후 구현. 최종은 `{type:"completion",finalText,usage}`(`result` 아님!), message/reasoning/tool_call 매핑 |
